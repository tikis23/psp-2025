...
export default function ItemsPage() {
    ...
    const loadTaxRates = async () => {
        if (!user?.merchantId) return
        try {
            const data = await getTaxRates(user.merchantId)
            setTaxRates(data)
        } catch (error) {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!user?.merchantId || !newItem.name || newItem.price <= 0 || !newItem.taxRateId) {
            toast.error("Please fill in all fields including tax rate")
            return
        }
