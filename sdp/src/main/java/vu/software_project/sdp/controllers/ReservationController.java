...
public class ReservationController {
    ...
    public ResponseEntity<?> createReservation(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(reservationService.createReservation(request, userDetails.getId(), request.getMerchantId()));
    ...
    public ResponseEntity<?> updateReservation(
        ...
        try {
            CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
            return ResponseEntity.ok(reservationService.updateReservation(id, request, userDetails.getId(), request.getMerchantId()));
        } catch (IllegalArgumentException ex) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .contentType(MediaType.TEXT_PLAIN)
                    .body(ex.getMessage());
